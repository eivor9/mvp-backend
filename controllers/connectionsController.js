...
const { getConnections, getOneConnection, createConnection, deleteConnection } = require("../queries/connections.js");
...
connections.delete("/:id", async (req, res) => {
    ...
    if (deletedConnection.id) {
        res.status(200).json({message: "Connection successfully deleted", ...deletedConnection});
    } else {
